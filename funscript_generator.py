"""
Funscript Generator - StashApp Plugin
...
def get_plugin_setting(key: str, default: Any = None) -> Any:
    ...
    try:
        settings = stash.get_configuration().get("plugins", {}).get("funscript_generator", {})
...
def get_trigger_tag() -> str:
    ...
    return get_plugin_setting("trigger_tag", "FunscriptGenerator_Process")


def get_complete_tag() -> Optional[str]:
    """Get the completion tag name."""
    tag = get_plugin_setting("complete_tag", "FunscriptGenerator_Complete")
    return tag if tag else None


def get_error_tag() -> str:
    """Get the error tag name."""
    return get_plugin_setting("error_tag", "FunscriptGenerator_Error")
